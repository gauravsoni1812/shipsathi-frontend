import React, { Component } from 'react';
import Link from 'next/link';

class OurLovingClients extends Component {
    render() {
        return (
            <section className="our-loving-clients ptb-100 bg-f4f5fe">
                <div className="container">
                    <div className="section-title">
                        <h2>Our Loving Clients</h2>
                    </div>

                    <div className="clients-logo-list align-items-center">
                        <div className="single-clients-logo">
                            <a href="#" target="_blank">
                                <img src="/images/clients/abiba.jpg" alt="image" />
                            </a>
                        </div>

                        <div className="single-clients-logo">
                            <a href="#" target="_blank">
                                <img src="/images/clients/deevaz.jpg" alt="image" />
                            </a>
                        </div>

                        <div className="single-clients-logo">
                            <a href="#" target="_blank">
                                <img src="/images/clients/puneet.svg" alt="image" />
                            </a>
                        </div>

                        <div className="single-clients-logo">
                            <a href="#" target="_blank">
                                <img src="/images/clients/pride.jpg" alt="image" />
                            </a>
                        </div>

                        <div className="single-clients-logo">
                            <a href="#" target="_blank">
                                <img src="/images/clients/puneet.svg" alt="image" />
                            </a>
                        </div>
                    
                    </div>
                </div>


                <div className="container">
                    <div className="section-title">
                        <h2>Carrier Partners</h2>
                    </div>

                    <div className="clients-logo-list align-items-center">
                        <div className="single-clients-logo">
                            <a href="#" target="_blank">
                                <img src="/images/clients/abiba.jpg" alt="image" />
                            </a>
                        </div>

                        <div className="single-clients-logo">
                            <a href="#" target="_blank">
                                <img src="/images/clients/deevaz.jpg" alt="image" />
                            </a>
                        </div>

                        <div className="single-clients-logo">
                            <a href="#" target="_blank">
                                <img src="/images/clients/puneet.svg" alt="image" />
                            </a>
                        </div>

                        <div className="single-clients-logo">
                            <a href="#" target="_blank">
                                <img src="/images/clients/pride.jpg" alt="image" />
                            </a>
                        </div>

                        <div className="single-clients-logo">
                            <a href="#" target="_blank">
                                <img src="/images/clients/puneet.svg" alt="image" />
                            </a>
                        </div>
                    
                    </div>
                </div>

            </section>
        );
    }
}

export default OurLovingClients;